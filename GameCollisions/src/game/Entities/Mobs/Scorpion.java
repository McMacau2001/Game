...
public class Scorpion extends Entity {
	...
	public Scorpion(int x, int y) {
		...
		sheet = new SpriteSheet(ImageManager.loadImage("scorpion.png"),10, 1);
		animation = new SpriteAnimation();
		
		walking_right = new Sprite(sheet, 1, 10, false);
		walking_left = new Sprite(sheet, 1, 10, true);

		animation.runSprite(walking_right, (int)speed+5, true);
	...
	public void update() {
		...
		if(getDirection().x()>0) 
			...
			animation.runSprite(walking_left, (int)speed+5, true);
